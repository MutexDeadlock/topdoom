...
import { MATERIALS, ROT0_BANK, drawnLumps, fxLayer, recordImpacts } from '../fixtures/spritestubs.ts';
...
function rig(): {
  ...
  const impacts = recordImpacts(effects);
  const ctx = {
    world,
    things: null,
  ...
  return { effects, projectiles, world, fireFrom: at, impacts };
}
...
describe('Projectiles · an impact sits off the wall, not on it', () => {
