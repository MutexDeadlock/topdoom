 ...
 * 320-wide view on purpose: a port at any resolution keeps this same index (`SCREENWIDTH /
 * viewwidth` in the table build), so the darkening is a property of the map, not the window.
 */
...
/**
 * The fake-contrast offset for a wall running from (ax,ay) to (bx,by) — the one true copy, so
 * `addWall` and `MoverGeometry.recolorSector` (which needs to redo this per-quad when a sector's
 * light changes at runtime) can't drift apart.
 */
...
/**
 * `lightToColor` plus `BRIGHTNESS_LIFT` (`constants.ts`) — what lights every sprite, the map
 * shader's `liftedGain` being the same sum for geometry. `lightToColor` itself stays pure and
...
/**
 * The one row of the view matrix `viewDepthAt` needs. Four numbers rather than a matrix: written
 * once a frame, read once per drawn sprite. Untouched it is the identity, whose depth is a plain
 * `-z` — what a layer drawn headless reads.
 */
let viewX = 0;
let viewY = 0;
let viewZ = 1;
