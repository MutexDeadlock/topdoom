...
function blitPatch(dst: Bitmap, patch: Patch, originX: number, originY: number, pal: Uint8Array): void {
...
export function readPicture(wad: Wad, name: string, pal: Uint8Array): Bitmap | null {
...
interface TextureDef {
  ...
  width: number;
  height: number;
  /** Patch lump names, already resolved through the right file's PNAMES. */
  patches: { originX: number; originY: number; patch: string }[];
}
...
/**
 * Collects TEXTURE1/TEXTURE2 from every loaded file, later definitions winning
 * by name.
 *
 * Vanilla treats a PWAD's TEXTURE1 as a full replacement, which is why such
 * PWADs are built to re-list every IWAD texture. Merging instead only ever adds
 * names that would otherwise resolve to nothing, so a PWAD combined with an
 * IWAD it was not built against still renders.
 *
 * Patch indices are relative to the PNAMES of the file that defined the
 * texture, so they are resolved to names here rather than stored raw.
 */
function readAllTextures(wad: Wad): Map<string, TextureDef> {
  ...
  for (const lump of wad.lumps) {
    ...
    // A file with textures but no PNAMES of its own borrows the last one seen.
...
export class GraphicsBank {
  ...
  constructor(wad: Wad) {
    ...
    // Flats live between F_START/F_END, with F1_/F2_ sub-ranges nested inside
    // and PWADs adding their own FF_START ranges. Later entries win.
  ...
  texture(name: string): Bitmap | null {
    ...
    if (def) {
      ...
      for (const p of def.patches) {
        ...
        if (!lump) continue;
        try {
          blitPatch(bmp, readPatch(this.wad.reader(lump)), p.originX, p.originY, this.palette);
        } catch {
