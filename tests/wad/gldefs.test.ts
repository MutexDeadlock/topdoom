...
describe('GLDEFS · the stock file', () => {
  // The source under `assets/`, not the `GLDEFS` lump the plugin folds it into — that build step is
  // `tests/wad/shipped.test.ts`'s.
  const STOCK = fileURLToPath(new URL('../../assets/gldefs.txt', import.meta.url));
