...
export class VoodooDolls {
  ...
  /** Scratch for the force queries' sector walk — see `World.sectorsTouching`. */
  ...
  update(dt: number, forces: Forces, cross: (prev: Pos2, doll: VoodooDoll) => TeleportDest | null): void {
    for (const doll of this.dolls) {
      ...
      const carry = forces.carryForBody(doll, PLAYER_RADIUS, this.touched);
      if (carry) {
        doll.momX += carry.x;
        doll.momY += carry.y;
      }
      ...
      const push = forces.pushForBody(doll, PLAYER_RADIUS, true, this.touched);
      if (push) {
        doll.momX += push.x;
        doll.momY += push.y;
      }
      ...
      if (doll.momX !== 0 || doll.momY !== 0) {
        ...
        const ground = forces.frictionUnder(doll, PLAYER_RADIUS, speed, this.touched);
