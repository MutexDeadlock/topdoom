...
function run(f: ReturnType<typeof loadBlock>, seconds: number): void {
  chaseFor(f.body, CACO, f.world, f.player, seconds);
}

describe('Monster AI · a floater straddling a block it cannot fit on', () => {
  ...
  for (const [name, type] of [
    ...
    test(`a ${name} gets out instead of hanging on the block forever`, () => {
      ...
      const moved = chaseFor(f.body, stats, f.world, f.player, 3);
