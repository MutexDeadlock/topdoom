...
weapons. It also breaks the reset model (§ Applying: reset, then patch), since a re-key can collide
with an existing key, so a reset would have to restore key *sets* and not just values.
