...
A nook beside a block, under its material, is a pocket like one beside a ring (§ above): the
block's faces onto it are its `buried` faces, and `pocketsOf` roofs it flush with the block's cap.
...
- **The top of a solid structure takes none of it** (`FlatSpec.wallShaded`): a lid and a pocket's
  ...
  it was just made flush with (§ Solid structures).
