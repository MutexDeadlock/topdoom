...
describe('Monster AI · a monster already hanging over a ledge', () => {
  ...
  test('walks instead of freezing, and never off the ledge', () => {
    ...
    let lowest = body.z;
    let westmost = body.x;
    chaseFor(body, stats, world, target, 30, {
      blockersFor: () => [],
      each: () => {
        travelled = Math.max(travelled, vecLength(body.x - start.x, body.y - start.y));
  ...
  test('stays on solid ground at an open ledge, where it has room to drift', () => {
    ...
    chaseFor(body, stats, world, target, 20, {
      blockersFor: () => [],
      each: (tic) => {
        assert.equal(body.z, 0, `stepped into the pit at frame ${tic}`);
        assert.ok(body.x >= edge, `centre crossed the ledge by ${(edge - body.x).toFixed(1)} units at frame ${tic}`);
      },
    });
  });
  ...
  test('a monster on flat ground still refuses to step onto the ledge', () => {
    ...
    chaseFor(body, stats, world, target, 8, {
      blockersFor: () => [],
      each: (tic) => assert.equal(body.z, 0, `stepped into the pit at frame ${tic}`),
    });
  });
});
