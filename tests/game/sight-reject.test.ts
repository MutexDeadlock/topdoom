...
import { World, hasLineOfSight } from '../../src/game/world.ts';
...
describe('world · REJECT', () => {
  ...
  test('only the marked pair is blinded, and only in the direction it is marked', () => {
    ...
    assert.equal(hasLineOfSight(world, at(3, 1), at(0, 0)), true);
  });
  ...
  test('a clear bit cannot grant sight the geometry refuses', () => {
    ...
    assert.equal(hasLineOfSight(world, { ...walled.centre(0, 0), z: 0 }, { ...walled.centre(2, 0), z: 0 }), false);
  });
});
