...
function chase(f: ReturnType<typeof corridor>, seconds: number, each?: (tic: number) => void): void {
  for (let i = 0; i < ticsIn(seconds); i++) {
    stepMonsterAI(f.body, stats, f.world, { dt: DOOM_TIC, target: f.target, targetRadius: PLAYER_RADIUS, targetHeight: PLAYER_HEIGHT, blockersFor: () => [] });
    each?.(i);
  }
}

describe('Regressions · a monster perched on a ledge corner over the floor', () => {
